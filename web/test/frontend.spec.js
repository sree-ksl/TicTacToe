...
describe("DipDappDoe frontend", async function () {
    ...
    it("should play a game created and won by the opponent", async function () {
        ...
        let hash = await DipDappDoe.methods.saltedHash(100, "player 2 salt").call()

        let tx = await DipDappDoe.methods.createGame(hash, "Jim").send({ from: player2, value: web3.utils.toWei("0.1", "ether") })
        ...
        expect(value).to.equal("Jim (0.1 Ξ)")
        ...
        tx = await DipDappDoe.methods.confirmGame(gameIdx, 100, "player 2 salt").send({ from: player2 })
        ...
        await page.waitForSelector('.ant-notification-notice-description')
        value = await page.$eval(".ant-notification-notice-description", node => node.innerText)
        expect(value).to.equal("Jim has confirmed the game!")

        await page.waitForSelector('#status')
        value = await page.$eval("#status", node => node.innerText)
        expect(value).to.equal(`Waiting for Jim`)
        ...
        await markPosition(gameIdx, 2, "Jim", page, "x")
        await markBrowserPosition(0, "Jim", page, metamask, "o")
        await markPosition(gameIdx, 4, "Jim", page, "x")
        await markBrowserPosition(3, "Jim", page, metamask, "o")
        
        // THE OPPONENT MARKS AND WINS

        tx = await DipDappDoe.methods.markPosition(Number(gameIdx), 6).send({ from: player2 })
        expect(tx.events.PositionMarked.returnValues.gameIdx).to.equal(gameIdx)
        await delay(200)

        await page.waitForSelector('#status')
        value = await page.$eval("#status", node => node.innerText)
        expect(value).to.equal("Jim is the winner of this game")
